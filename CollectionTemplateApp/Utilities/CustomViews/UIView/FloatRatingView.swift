...
/// A simple rating view that can set whole, half or floating point ratings.@IBDesignable
...
open class FloatRatingView: UIView {
    ...
    /// Float rating view type
     open var type: FloatRatingViewType = .halfRatings
    ...
    private func refresh() {
        for i in 0..<fullImageViews.count {
            ...
            if rating >= Double(i+1) {
                ...
                maskLayer.frame = CGRect(x: 0, y: imageView.frame.size.height*0.3, width: imageView.frame.size.width, height: imageView.frame.size.height)
